from aiogram.utils.keyboard import InlineKeyboardBuilder
from keyboards.inline_keyboards.base_buttons import add_button_back, add_button_menu

one_c_buttons = ('Создание Заказа Покупателя и выставление счёта.',
                 'Создание отгрузочной накладной',
                 'Возврат от покупателя',
                 'Создание контрагента',
                 'Создание номенклатуры')


def build_one_c_kb():
    builder = InlineKeyboardBuilder()
    for i, text in enumerate(one_c_buttons):
        builder.button(text=text, callback_data=f'command_0_{i}')
    add_button_back(builder)
    add_button_menu(builder)
    builder.adjust(1)
    return builder.as_markup()
